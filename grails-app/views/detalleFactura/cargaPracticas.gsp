...
<body>
<a href="#create-detalleFactura" class="skip" tabindex="-1"><g:message code="default.link.skip.label"
...
<div class="nav" role="navigation">
    <ul>
        ...
        <li><g:link class="create" controller="planillaInternacion" action="index"
                    id="${detalleFacturaInstance?.planillaInternacion?.id}"><g:message
                    code="defaul.button.volver.label"/></g:link></li>
    </ul>
</div>
...
<g:form id="formulario" url="[resource: detalleFacturaInstance, action: 'saveCarga']">
    <fieldset class="form">
        ...
        <div class="fieldcontain ${hasErrors(bean: detalleFacturaInstance, field: 'planillaInternacion', 'error')} required">
            ...
            <g:select id="planillaInternacion" name="planillaInternacion.id"
                      from="${maternidad.PlanillaInternacion.findAllById(detalleFacturaInstance?.planillaInternacion?.id)}"
                      optionKey="id" required="" value="${detalleFacturaInstance?.planillaInternacion?.id}"
                      class="many-to-one"/>
        ...
        <div class="fieldcontain ${hasErrors(bean: detalleFacturaInstance, field: 'plan', 'error')} required">
            ...
            <g:select id="plan" name="plan.id" from="${maternidad.Plan.findAllById(detalleFacturaInstance?.plan?.id)}"
                      optionKey="id" required="" value="${detalleFacturaInstance?.plan?.id}" class="many-to-one"/>
        ...
        <div class="fieldcontain ${hasErrors(bean: detalleFacturaInstance, field: 'cantidad', 'error')} required">
            ...
            <g:field type="number" step="any" id="cantidad" name="cantidad" required=""
                     value="${fieldValue(bean: detalleFacturaInstance, field: 'cantidad')}"/>

        </div>


        <div id="divgasto"
             class="fieldcontain ${hasErrors(bean: detalleFacturaInstance, field: 'valorGastos', 'error')} ">
        ...
        <div id="divhonorario"
             class="fieldcontain ${hasErrors(bean: detalleFacturaInstance, field: 'valorHonorarios', 'error')} ">
            <label for="valorHonorarios">
                <g:message code="detalleFactura.valorHonorarios.label" default="Valor Honorarios"/>

            </label>
            <g:field type="number" step="any" name="valorHonorarios"
                     value="${detalleFacturaInstance?.valorHonorarios?.toString()}"/>
        ...
        <!--
            <div class="fieldcontain ${hasErrors(bean: detalleFacturaInstance, field: 'valorPractica', 'error')} ">
                ...
                <g:field name="valorPractica"
                         value="${fieldValue(bean: detalleFacturaInstance, field: 'valorPractica')}"/>
...
-->
    ...
    <fieldset class="buttons">
        <g:submitButton id="boton" name="create" class="save"
...
<table>
    ...
    <g:each in="${listaDetalles}" status="i" var="detalleFactura">
        <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
            ...
            <td>
                <g:link class="edit" action="editDetalle" params="[id: detalleFactura?.id]"><g:message
